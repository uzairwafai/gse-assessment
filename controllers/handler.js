...
const resolveContact = async (req, res) => {
  ...
  if (matches.length === 0) {
    const newContact = await Contact.create({ email, phone, isPrimary: true });
    return res.json({
  ...
  const recursiveRelationfind = async (contact) => {
    ...
    contactMap.set(contact.id, { email: contact.email, phone: contact.phone });
...
//if any of them is not present then create a new record
  if (!emailExists || !phoneExists) {
    const newContact = await Contact.create({ email, phone, isPrimary: false });
